...
describe('# User Request', () => {
    context('# SignIn Request', () => {
        describe('When request signin',() => {
            before(async () => {
                await db.User.create({ name: 'tes1', email: 'test1@example.com', password: bcrypt.hashSync('12345678', bcrypt.genSaltSync(10), null), admin: true })
            })
            ...
            it('should return 401 and get error message', done => {
                request(app)
                    ...
                    .end((err, res) => {
                        expect(res.body.status).to.equal('error');
                        expect(res.body.message).to.equal('no such user found or passwords did not match');
                        done();
                    });
            });
